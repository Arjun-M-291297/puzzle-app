...
interface Props {
  ...
  onClose: () => void;
  clues: ClueEntry[];
  revealOnceIds: Set<string>;
}

export function NotebookSheet({ visible, onClose, clues, revealOnceIds }: Props) {
        ...
        <Animated.View style={[styles.sheet, { transform: [{ translateY }] }]}>
          <Pressable onPress={(e) => e.stopPropagation()}>
            ...
            <ScrollView style={styles.list} showsVerticalScrollIndicator={false}>
              ...
              {clues.map((clue) => {
                const hidden = revealOnceIds.has(clue.id);
                return (
                  <View key={clue.id} style={styles.entry}>
                    ...
                    <View style={styles.entryText}>
                      ...
                      {hidden ? (
                        <Text style={styles.entryHidden}>You'll have to remember the details.</Text>
                      ) : (
...
const styles = StyleSheet.create({
  ...
  entryHidden: { color: colors.rustBright, fontSize: 12, fontStyle: 'italic' },
});
